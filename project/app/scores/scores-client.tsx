...
export default function ScoresClient() {
  ...
  const data = useMemo(() => {
    ...
    const filtered = records.filter(
      (r) =>
        r.shgName.toLowerCase().includes(query.toLowerCase()) ||
        r.videoName.toLowerCase().includes(query.toLowerCase())
    );
              ...
              {data.filtered.map((r, i) => (
                <Card key={`${r.shgName}-${r.videoName}-${i}`} className="surface-card score-card-interactive">
                  <CardContent className="p-5 space-y-3">
                    ...
                    <p className="text-xs text-slate-500">Score classification based on overall AI evaluation.</p>
                  </CardContent>
                </Card>
              ))}
            </div>
